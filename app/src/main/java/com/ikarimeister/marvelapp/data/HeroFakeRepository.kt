...
class HeroFakeRepository @Inject constructor(): Repository<Hero,String> {

    private val data = fakeData().toMutableMap()

    override fun get(key: String): Hero? = data[key]

    override fun plus(item: Hero) {
        data[item.id] = item
    }

    override fun minus(item: Hero) {
        data.remove(item.id)
    }
    ...
    private fun fakeData(): Map<String,Hero> {
        return listOf(
            Hero(
                id = "ScarletWitch",
                name = "Scarlet Witch",
